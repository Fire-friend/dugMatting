...
class RandomCrop_city(object):  # used for results in the CVPR-19 submission
    ...
    def __call__(self, img, mask):
        ...
        pad_h = max(self.size[0] - img_h, 0)
        pad_w = max(self.size[1] - img_w, 0)
        ...
        c1,c2 = img.crop((x1, y1, x1 + tw, y1 + th)), mask.crop((x1, y1, x1 + tw, y1 + th))
        return (
            c1,c2
        )
...
class ColorJitter(object):
    def __call__(self, img, mask):
        ...
        if num < 0.5:
            img = Image.fromarray(img, mode="RGB")
            # color_transform = transforms.ColorJitter((0.05, 0.15), (0.05, 0.15), (0.05, 0.15), (0.05, 0.15))
            color_transform = transforms.ColorJitter(brightness=0.25, contrast=0, saturation=0, hue=0)
...
class Gaussian(object):
    def __call__(self, img, mask):
        ...
        return np.array(img, dtype='uint8'), mask
...
class RandomRotate(object):
    ...
    def __call__(self, img, mask):
        ...
        if mask is not None:
            ...
            return (
                np.array(img.rotate(rotate_degree, Image.BILINEAR)),
                np.array(mask.rotate(rotate_degree, Image.NEAREST)),
            )
        else:
            rotate_degree = random.random() * 2 * self.degree - self.degree
            return (
