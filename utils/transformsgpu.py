...
def colorJitter(colorJitter, img_mean, data=None, target=None, s=0.25):
    ...
    if not (data is None):
        if data.shape[1] == 3:
            if colorJitter > 0.2:
                s1 = random.uniform(0.15, 1.25)
                s2 = random.uniform(0.15, 1.25)
                s3 = random.uniform(0.15, 1.25)
                s4 = random.uniform(0, 0.25)
                # img_mean, _ = torch.broadcast_tensors(img_mean.unsqueeze(0).unsqueeze(2).unsqueeze(3), data)
                # seq = nn.Sequential(kornia.augmentation.ColorJitter(brightness=s1, contrast=s2, saturation=s3, hue=s4))
                seq = nn.Sequential(kornia.augmentation.ColorJitter(brightness=s, contrast=s, saturation=s, hue=s))

                img_mean, _ = torch.broadcast_tensors(
                    torch.from_numpy(np.array([0.55564057, 0.52342629, 0.54125332])).cuda().unsqueeze(0).unsqueeze(2).unsqueeze(3), data)
                img_sigma, _ = torch.broadcast_tensors(
                    torch.from_numpy(np.array([0.34888856, 0.31294156, 0.30838196])).cuda().unsqueeze(0).unsqueeze(2).unsqueeze(3), data)
...
def flip(flip, data=None, target=None):
    # Flip
    if flip == 1:
        if not (data is None): data = torch.flip(data, (
        3,))  # np.array([np.fliplr(data[i]).copy() for i in range(np.shape(data)[0])])
...
def mix(mask, data=None, target=None):
    ...
    if not (target is None):
        ...
        if mask.shape[1] != target.shape[2]:
            mask = nn.functional.upsample_bilinear(mask.unsqueeze(1).float(), size=(target.shape[2], target.shape[3])).squeeze(1).long()

        # mask = torch.tensor(0).resize(target.shape)
        target = torch.cat(
